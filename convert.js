...
export function tojs(v) {
  ...
  if (v.t == 3) {
    var r = [];
    var same = true;
    for (var z = 0; z < v.v.length; z++) {
      r[z] = tojs(v.v[z]);
      same &= v.v[z].t == v.v[0].t;
    }
    if (same && v.v.length != 0 && v.v[0].t == 1) {
      return r.join("");
    }
    return r;
  }
  if (v.t == 4) {
    var r = {};
    for (var z = 0; z < v.k.v.length; z++) {
      r[tojs(v.k.v[z])] = tojs(v.v.v[z]);
    }
    return r;
  }
  throw new Error(
    "cannot convert '" + JSON.stringify(v) + "' to a JavaScript datatype.",
  );
}
...
function trampoline(env, name, args, body) {
  ...
  for (var z = 0; z < args.length; z++) {
    arguse.push({ t: 7, v: args[z] });
  }
  env.put(ks(name), true, {
    t: 5,
    args: args,
    v: [{ t: 8, v: name, curry: arguse }],
  });
}
